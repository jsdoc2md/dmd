...
class Dmd extends DocletTemplate {
  ...
  groupBy (groupByProperties) {
    groupByProperties = arrayify(groupByProperties)
    for (let doclet of this) {
      if (doclet.children.length) {
        groupByProperties.forEach(groupByProperty => {
          const groups = getDistinct(doclet.children, groupByProperty)
          /* minimum two groups */
          if (groups.length >= 2) {
            doclet.groupChildren(groups, groupByProperty)
          }
        })
      }
    }
  ...
  groupChildren (groups, groupBy) {
    /* for each group, create a node and move children to it from this */
    groups.forEach(groupName => {
      var group = Dmd.create(
        { id: groupName, kind: 'group' },
        require('./decoration/heading-depth').bind(null, 2)
      )
      this.add(group)

      const scopeChildren = this.children.filter(d => d.data[groupBy] === groupName)
      scopeChildren.forEach(d => {
        group.add(d)
        this.remove(d)
      })
    })
...
/**
 * get an array of unique values from the given property
 * @param {object[]}
 * @param {string} - property name
 * @returns {string[]}
 */
function getDistinct (array, property) {
  return array.map(d => d.data[property]).reduce((prev, curr) => {
    if (curr && prev.indexOf(curr) === -1) prev.push(curr)
    return prev
  }, [])
}
