...
@property [template] {string} - A handlebars template to insert your documentation into, the default is `"{{>main}}"`.
@property [heading-depth] {number} - the heading depth to begin the docs from. For example, `2` (the default) starts from a markdown heading of `"##"`).
...
@property [name-format] {string} - specify `code` to format identifier names as code
...
@property [separators] {boolean} - Put <hr> breaks between identifiers. Improves readability on bulky docs. 
