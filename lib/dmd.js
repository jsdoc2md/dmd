...
function dmd(options){
    ...
    if(options["source-links"]) {
        ddata.setAddSourceLinks(options["source-links"]);

        if(options["base-url"]) {
            // Add trailing slash to avoid issues with url.resolve collapsing last url fragment
            if(options["base-url"].substr(-1) !== '/') {
               options["base-url"] = options["base-url"] + '/';
            }

            ddata.setBaseUrl(options["base-url"]);
        }
    }
...
dmd.cliOptions = [
    ...
    { name: "source-links", type: Boolean,
      description: "Include source links in the headers"
    },
    { name: "base-url", type: String,
      description: "Repo base url for source links"
    }
];
