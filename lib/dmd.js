...
[ "../helpers/helpers", "../helpers/locale", "ddata", "handlebars-string", "handlebars-regexp", "handlebars-comparison", "handlebars-json", "handlebars-array" ].forEach(function(modulePath){
...
function DmdOptions(){
    ...
    * ```
    * the equivalent operation using the command-line tool:
    * ```
...
function dmd(options){
    ...
    /* locale overrides */
    if (options.locale) locale.extendCatalogue(options.locale);
    ...
    return tr.connect(
        tr.collectJson(function(json){
            return new Identifiers(json);
            // refactor so all the partials take an ArrayTools as the input, not an array.
        }),
        handlebars.createCompileStream(
            options.template,
            {
                objectMode: true,
                preventIndent: true,
...
dmd.cliOptions = [
    ...
    { name: "helper", type: String, multiple: true,
      description: "handlebars helper files to override or extend the default set"
    },
    { name: "partial", type: String, multiple: true,
